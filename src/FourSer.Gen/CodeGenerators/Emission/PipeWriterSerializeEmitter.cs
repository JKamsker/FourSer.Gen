...
internal static class PipeWriterSerializeEmitter
{
    public static void Emit(IndentedStringBuilder sb, MethodPlan plan)
    {
        var type = plan.Facts.Type;
        sb.WriteLineFormat("public static void Serialize({0} obj, global::System.IO.Pipelines.PipeWriter writer)", type.Name);
        using (sb.BeginBlock())
        {
